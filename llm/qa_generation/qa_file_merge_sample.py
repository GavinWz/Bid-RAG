...
def merge_json_files(source_folder, target_folder, dataset_info_file):
    ...
    output_filename = f"{parent_folder_name}-{base_filename}-outputQA-sample.json"
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                try:
                    ...
                    if len(data) >= 2:
                        ...
                        sampled_data = data  # 如果数据不足两个，则全部抽取
                    # 将抽取的数据合并到总列表中
    ...
    dataset_info = {
        f"{parent_folder_name}-{base_filename}-outputQA-sample": {
