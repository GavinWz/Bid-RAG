...
from rag.sql_answer_test import SqlModule
from rag.rag_answer_test import RagModule
...
class ChatBotGradio():
    def __init__(self, config, model, client, embedding_model):
        ...
        self.js_shift_enter_newline = """
            ...
            """    
        self.rag_module = RagModule(config, model, client, embedding_model)
        self.sql_module = SqlModule(config, model, client, embedding_model, self.rag_module)
    ...
    def predict_stream(self, user_input, chat_history):
        ...
        # 调用模型生成回复
        response_chunks = self.sql_module.answer(user_input, self.messages, True)
        # response_chunks = self.rag_module.answer_with_rag(user_input, self.messages, True)
        ...
        self.is_generating = False
        self.messages.append(AIMessage(content=partial_response))
