...
def insert_dynamic_table(connection, table_name, project_id, data_dict):
    ...
    for k in data_dict.keys():
        if k not in existing_columns:
            if k in {"附件"} or "正文" in k:
                alter_sql = f"ALTER TABLE `{table_name}` ADD COLUMN `{k}` LONGTEXT;"
            else:
                alter_sql = f"ALTER TABLE `{table_name}` ADD COLUMN `{k}` VARCHAR(255);"
            with connection.cursor() as cursor:
    ...
    columns = ["项目ID"] + list(data_dict.keys())
    col_str = ", ".join([f"`{col}`" for col in columns])
    placeholders = ", ".join(["%s"] * len(columns))

    # 对应的值，第一个是 project_id，其它从 data_dict 里取
    values = [project_id] + [str(data_dict[k]) for k in data_dict.keys()]
...
def main():
    ...
    try:
        ...
        # 3. 读取 JSONL 文件并写入
        with open("安徽合肥公共资源交易中心-政府采购.jsonl", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
